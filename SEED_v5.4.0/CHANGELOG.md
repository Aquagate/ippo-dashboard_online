## [5.4.0] - 2026-02-18
### ☁️ Cloud Sync & Auth Pattern Edition
**"個人データをクラウドで守り、どこでも使える自由を"**

Ippo Dashboard Online から、MSAL 2.x (Redirect Flow) を用いた「サーバーレス・クラウド同期」と、UIとロジックを分離する「Observerパターン」を統合しました。

### Added
- **OneDrive MSAL Wrapper**: `ops/weapons/auth/onedrive_msal.ts`
  Redirect Flowに対応した、モダンで堅牢なMicrosoft Graph API認証ラッパー。
- **Sync Observer Pattern**: `ops/snippets/patterns/sync_observer.ts`
  同期状態（接続、送信中、完了、エラー）をUIから分離して通知するデザインパターン。
- **Case Study**: `ARCHIVED_LEARNINGS/IPPO_ONLINE_CASE_STUDY.md`
  モノリスHTMLからモジュラーTypeScriptへの移行と、同期アーキテクチャの事例。
