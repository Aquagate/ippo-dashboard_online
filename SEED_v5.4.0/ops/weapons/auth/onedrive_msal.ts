/**
 * Weapon: OneDrive MSAL Wrapper
 * Origin: Ippo Dashboard Online (SEED v5.3)
 * Description: Reusable wrapper for Microsoft Graph API using MSAL 2.x with Redirect Flow.
 * 
...
const DEFAULT_FILE_PATH = "/Apps/YourApp/data.json";
...
async function odGetToken(config: OdAuthConfig): Promise<string> {
    if (!odMsalApp) await odEnsureMsal(config);
    if (!odAccount) throw new Error("Not signed in.");
    try {
...
export async function odGetRemoteData(config: OdAuthConfig, filePath: string): Promise<{ data: any; etag: string | null }> {
    ...
    if (!res.ok) {
        ...
        throw new Error(`OneDrive Download Failed: ${res.status} ${t}`);
    }
    const etag = res.headers.get("ETag");
    const raw = await res.text();
    let data;
    try { data = JSON.parse(raw); } catch {
        throw new Error("Invalid JSON in OneDrive.");
    }
    return { data, etag };
}
...
export async function odPutRemoteData(config: OdAuthConfig, filePath: string, data: any, etag: string | null): Promise<string | null> {
    ...
    if (!res.ok) {
        const t = await res.text().catch(() => "");
        throw new Error(`OneDrive Upload Failed: ${res.status} ${t}`);
    }
